...
class Api
{
 ...
 try {
    if (!is_readable($localPath)) {
        $this->helper->log("File is not readable: {$localPath}", 'error');
        return false;
    }
    
    $fileContent = file_get_contents($localPath);
    if ($fileContent === false) {
        $this->helper->log("Failed to read local file: {$localPath}", 'error');
        return false;
    }
} catch (\Exception $e) {
    $this->helper->log("Exception reading file: {$localPath}. Error: " . $e->getMessage(), 'error');
    return false;
}
